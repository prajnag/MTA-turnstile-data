...
In terms of technicalities, the data is collected through electronic turnstiles that register each entry and exit, timestamped to the nearest hour. The MTA uses this information for operational purposes and releases it for public use under the state's open data initiative.
...
Careful documentation of the data source is essential for reproducibility and verification of the research. The specific dataset can be accessed through the provided link: MTA Subway Hourly Ridership Data (https://data.ny.gov/Transportation/MTA-Subway-Hourly-Ridership-Beginning-February-202/wujg-7c2s). This link leads directly to the New York State open data portal where the dataset is hosted, allowing for direct download and inspection of the data.
...
- Our team will carry out preliminary cleaning to maintain data quality. This includes addressing missing values, verifying and correcting data types, and resolving any anomalies resulting from maintenance or special events.
...
Given the nature of our research questions, which prioritize the analysis of subway traffic patterns, station popularity, and peak travel times, the geographical variables—New.York.Zip.Codes and Counties—are peripheral. Consequently, the decision has been made to exclude these variables from the dataset. This strategic exclusion is aimed at streamlining the analysis, ensuring clarity, and maintaining focus on the variables most critical to our research objectives.
